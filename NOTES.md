...
- **Admin download reissue** (`/api/admin/orders/reissue`) — `@gigamusic/checkout.sendPurchaseConfirmation` is the wrong seam for the support flow: it swallows provider errors and returns `void`, so an admin pressing the button couldn't be told whether the mail went out or be handed the link when it didn't. The route sends via `emailProvider()` directly (same shape as `sendOrderLookupEmail`), but the *rendering* still composes `@gigamusic/email.renderPurchaseConfirmation` — `src/lib/email.ts:renderDownloadReissue` swaps only the "Thank you for your purchase!" sentence, so branding/layout/item formatting can't drift from the original email. `tests/lib/email-reissue.test.ts` fails if that upstream sentence ever changes.
