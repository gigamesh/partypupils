 ...
 *
 * Parallelism numbers are kept modest because the local dev Postgres
...
describe("consumeRateLimit", () => {
  it("honours the cap exactly under concurrent calls on the same key", async () => {
    ...
    const row = await db.query.rateLimits.findFirst({
      where: eq(rateLimits.key, "burst:single-key"),
    });
  ...
  it("resets the counter when the window has expired", async () => {
    ...
    await db
      .update(rateLimits)
      .set({ windowStart: new Date(Date.now() - 120_000) }) // 2 minutes ago
      .where(eq(rateLimits.key, "expiry"));
  ...
  it("creates the row on the first call", async () => {
    const before = await db.query.rateLimits.findFirst({
      where: eq(rateLimits.key, "first"),
    });
    expect(before).toBeUndefined();

    expect(await consume("first", 3, 60_000)).toBe(true);

    const after = await db.query.rateLimits.findFirst({
      where: eq(rateLimits.key, "first"),
    });
